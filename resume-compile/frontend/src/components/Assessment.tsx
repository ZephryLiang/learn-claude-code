...
import { assessResume } from "@/lib/api";
...
export default function Assessment({ resumeText, jdText, cachedResult, onResult, modelId, isLoading, onLoadingChange }: Props) {
  ...
  const handleAssess = async () => {
    ...
    setError(null);
    try {
      const res = await assessResume(resumeText, jdText, modelId);
      onResult(res.assessment);
    } catch (e: any) {
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      <h3 className="text-sm font-medium text-foreground">简历评估</h3>
      <p className="text-xs text-muted-foreground">基于当前简历和 JD，评估拿到面试的概率。</p>
      {!jdText.trim() && (
